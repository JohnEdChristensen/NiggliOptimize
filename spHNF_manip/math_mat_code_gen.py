...
def print_pg_as_mat(label, size):
    ...
    for i in range(0,size):
        print label + ": " + str(i)
        temp = transforms[indicies[i]]
        print temp
        try:
            print niggli_lat_id.niggli_id(numpy.transpose(temp))
        except:
            print "NIGGLI_LAT_ID ERROR"
...
def print_pg_one_as_mat(label, size):
    ...
    for i in range(0,size):
        print label + ": " + str(i)
        temp = transforms[indicies[URT_indicies[i]]]
        print temp
        try:
            print niggli_lat_id.niggli_id(numpy.transpose(temp))
        except:
...
def print_simple_pg_as_mat(label, size):
    ...
    for i in range(0,size):
        print label + ": " + str(i)
        temp =  transforms[indicies[simple_indicies[i]]]
        print temp
        try:
            print niggli_lat_id.niggli_id(numpy.transpose(transforms[indicies[simple_indicies[i]]]))
        except:
            print "NIGGLI_LAT_ID ERROR"
