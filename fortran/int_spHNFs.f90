    ...
    do i =1,nds
      ...
      if (MOD(f,c)==0)then
        ...
        do j=1,size(es)
          e = es(j)
          if(MOD(e,c)==0)then
            gamma21 = -c+e*e/real(c)
            if(MOD(gamma21,real(f))==0)then
              do d=0,f-1
                gamma13 = a+2*d
                if(MOD(gamma13,f)==0)then
                  do b=0,c-1
                    ...
                    if(MOD(beta12,c)==0)then
                      gamma12 = -b+d-(beta12*e/real(c))
                      if(MOD(gamma12,real(f))==0)then
                        nhnfs = nhnfs + 1
...
use sp_hnfs
  ...
  open (unit = 1, file = 'int_sphnfs.txt', form='formatted')
  write(*,*), arrSize
  do i = 1,arrSize
